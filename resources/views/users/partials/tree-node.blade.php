...
<div class="relative {{ $depth > 0 ? 'ml-5 border-l-2 border-brand-100 pl-5 sm:ml-8 sm:pl-8' : '' }}">
    ...
    <div class="group relative mb-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-3 transition hover:-translate-y-0.5 hover:border-brand-300 hover:bg-white hover:shadow-md sm:p-4">
        <a href="{{ route('users.profile', $user) }}" class="flex gap-3">
            ...
            <div class="min-w-0 flex-1">
                <div class="flex flex-wrap items-start justify-between gap-2">
                    <div class="min-w-0">
                        <p class="truncate text-base font-bold text-ink group-hover:text-brand-700">{{ $user->displayName() }}</p>
                    </div>
        ...
        @unless($user->isSuperAdmin())
            <div class="mt-3 flex flex-wrap gap-2 border-t border-slate-100 pt-3 text-xs">
