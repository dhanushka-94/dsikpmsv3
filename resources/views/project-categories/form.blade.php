...
@section('content')
    <form method="POST" action="{{ isset($category) ? route('project-categories.update', $category) : route('project-categories.store') }}" class="mx-auto max-w-2xl space-y-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
        @csrf
        ...
        <div class="flex gap-3">
            ...
            <a href="{{ route('project-categories.index') }}" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600">Cancel</a>
        </div>
    </form>
@endsection
