...
@section('page-subtitle', 'Manage categories used by projects')

@section('actions')
    <a href="{{ route('project-categories.create') }}" class="rounded-2xl bg-brand-600 px-4 py-2.5 text-sm font-bold text-white hover:bg-brand-700">Add category</a>
@endsection
        ...
        <table class="min-w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($categories as $category)
                    <tr>
                        <td class="px-4 py-3">
                            <span class="inline-flex h-8 min-w-8 items-center justify-center rounded-xl bg-slate-100 px-2 text-xs font-bold">{{ $category->sort_order }}</span>
                        </td>
